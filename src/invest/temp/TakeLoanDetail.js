...
class DetailScreen extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      inputs:[
        ...
        {label:L('Creditor Name'),key:'creditor',value:'',mandatory:true},
  ...
  componentDidMount() {
    ...
    if(mode === 'add') {
      inputs[0].value = lib.getOfflineId('takeloan',login.idmsuser);
  ...
  handleSave() {
    ...
    SqliteInvest.upsertTakeLoan(json.offlineID,json.creditor,json.amount,json.notes,json.ts,json.synch,json.trxoperation,json.trxdate,json.idmssupplier,json.idmsuser)
      .then(result=>{
        console.warn(result);
  ...
  handleRemove() {
    ...
    SqliteInvest.deleteTakeLoan(offlineID,idmssupplier,idmsuser)
      .then(result=>{
        console.warn(result);
        return UIHelper.getMainPageData();
      })
