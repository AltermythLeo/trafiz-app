...
class DetailScreen extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      inputs:[
        ...
        {label:L('New Unit'),key:'sellUnitName',value:'',mandatory:true},
        {label:L('Price per Unit'),key:'sellUnitPrice',value:'',mandatory:true,ctl:'numeric'},
        {label:L('Num Unit'),key:'numUnit',value:'',ctl:'numeric'},
  ...
  componentDidMount() {
    ...
    if(mode === 'add') {
      inputs[0].value = lib.getOfflineId('sellfish',login.idmsuser);
  ...
  handleSave() {
    ...
    json.deliveryOfflineID = ''; // todo
    json.buyFishOfflineID = ''; // todo
    const j = json;

    SqliteInvest.upsertSellFish(j.offlineID,j.deliveryOfflineID,j.buyFishOfflineID,j.sellUnitName,j.sellUnitPrice,j.amount,j.notes,j.ts,j.synch,j.trxoperation,j.trxdate,j.idmssupplier,j.idmsuser)
      .then(result=>{
        console.warn(result);
  ...
  handleRemove() {
    ...
    SqliteInvest.deleteSellFish(offlineID,idmssupplier,idmsuser)
      .then(result=>{
        console.warn(result);
        return UIHelper.getMainPageData();
      })
