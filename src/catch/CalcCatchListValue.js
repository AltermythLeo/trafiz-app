...
class ListScreen extends React.Component {
  ...
  componentDidMount() {
    ...
    const buyPrice = this.props.navigation.getParam('buyPrice',0);
    ...
    const totalprice = Number(buyPrice);
    ...
    if(!pid) pid = data.idmsbuyersupplier;
    ...
    if(loanIndex == -1) {
      if(data.idfishermanoffline) loanIndex = _.findIndex(loans,{'idfishermanoffline':data.idfishermanoffline});
      else if(data.idbuyeroffline) loanIndex = _.findIndex(loans,{'idbuyeroffline':data.idbuyeroffline});
    }

    console.warn('borrower at index:',loanIndex);
    console.warn(data);
  ...
  handleSave() {
    ...
    json['noteparam'] = this.state.notes;
    ...
    this.props.actions.editCatchListHaveNotes(json,offlineJson)   
    .then(()=>{
      return this.props.actions.getCatchFishes();
    })
    .then(()=>{
      this.setState({show:'form'});
      return this.props.navigation.navigate('CatchCatchListScreen');
    })
    .catch(err=>{
      console.warn(err);
      this.setState({show:'form'});      
  ...
  renderCalcResult(netBuyPrice) {
    return (
      <View style={{backgroundColor:'white',borderBottomWidth:1,borderColor:'gainsboro',elevation:1}}>
        ...
        <View style={{minHeight:64,padding:10,justifyContent:'center'}}>
          <TextField
            tintColor={lib.THEME_COLOR}
            label={L('Notes')}          
            multiline={true}
  ...
  renderLoanPanel(totalItems,totalPrice) {
    ...
    const loanIndexMinusOne = (this.state.loanIndex == -1);
    let btnDisabled = (disabled || loanExpenseDisabled || loanIndexMinusOne);
    return (
      <View style={{backgroundColor:'white',borderBottomWidth:1,borderColor:'gainsboro',elevation:1}}>
        ...
        <View style={{padding:10,flexDirection:'row',borderBottomWidth:1,borderColor:'gainsboro'}}>
          ...
          <View style={{}}>
            <Button disabled={disabled} style={{flex:1,container:{flex:1}}} raised text={L('Detail')} onPress={()=>this.enterDetail()} />
          </View>
        </View>
